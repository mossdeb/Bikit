...
import { useProLocale } from "@/components/pro-locale";
 ...
 *
 * `locale` is the reader's language for the errors — the download's and
 * the parser's — which are written on screen.
 */
...
> {
  const whole = await loadWholeImuSession(
    storagePath,
    mountOrientation,
    locale,
  );
...
export async function loadWholeImuSession(
  ...
  { data: ImuSessionData; error: null } | { data: null; error: string }
> {
  const t = getProDictionary(locale).importing.load;
  ...
  if (error || !blob) {
    return {
      data: null,
      error: t.downloadFailed(error?.message ?? t.noResponse),
    };
  }
  ...
  const result = parseImuBytes(await blob.arrayBuffer(), locale);
 ...
 * recording before the trim, for the dialog that sets it. The language
 * for the errors is read off the Pro locale provider, which every page
 * under /pro sits inside — the caller passes nothing. */
export function useImuSession(
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const result = await loadWholeImuSession(
        storagePath,
        mountOrientation,
        locale,
      );
  ...
  }, [storagePath, mountOrientation, locale]);
