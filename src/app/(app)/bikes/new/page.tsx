...
export default async function NewBikePage({
  ...
  const smartSetupCard = (
    <SmartSetupLink
      title={dict.bikes.aiSetup.entryTitle}
      description={dict.bikes.aiSetup.entryDescription}
      beta={dict.bikes.aiSetup.beta}
    />
  );
  ...
  const step1 = (
    <Fragment key="step-1">
      ...
      <div className="grid grid-cols-2 gap-5 sm:contents">
        <div className="space-y-1.5">
          <Label htmlFor="version">{dict.bikes.form.version}</Label>
          <Input id="version" name="version" placeholder={dict.bikes.form.versionPlaceholder} />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="year">{dict.bikes.form.yearRequired}</Label>
          {/* Same bounds as Smart Setup: below 2010 there are no catalogs to
              speak of, and one year ahead covers next season's models. */}
          <Input
            id="year"
            name="year"
            type="number"
      ...
      {/* Desktop only — on mobile the same card is handed to the wizard as
          step1Footer so it sits against the Next button. */}
      {aiSetupAvailable && <div className="hidden sm:col-span-2 sm:block">{smartSetupCard}</div>}
    </Fragment>
  );
  ...
  const step2 = (
    <Fragment key="step-2">
      <div className="space-y-1.5">
        ...
        <NativeSelect id="type" name="type" defaultValue="Enduro" required>
          {BIKE_TYPES.map((type) => (
      ...
      {/* Out of the optional-details dialog: this one is not decoration. It
          places the bike on the Bike Log, it is the date a component can
          inherit as its install date, and it is what the warranty expiry
          counts from. Hidden behind a dialog, almost no bike had it — and
          the reader never learned that those three were off because of it. */}
      <div className="space-y-1.5 sm:col-span-2">
        <Label htmlFor="purchase_date">{dict.bikes.form.purchaseDate}</Label>
        <Input id="purchase_date" name="purchase_date" type="date" />
        <p className="text-xs text-muted-foreground">{dict.bikes.form.purchaseDateHint}</p>
      </div>
      ...
      {/* The year and the purchase date are asked for above now, so they
          leave the picker — the same name twice in one form would submit
          two values. What stays is what is genuinely decoration. */}
      <BikeOptionalFields labels={bikeOptionalFieldLabels(dict)} omit={["year", "purchase_date"]} />
    </Fragment>
  );
