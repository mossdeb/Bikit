...
export default async function ImuLabPage() {
  ...
  const [{ data: sessions }, { data: bikes }] = await Promise.all([
    ...
    supabase.from("bikes").select("id, name, type").eq("user_id", userId).order("name"),
  ]);
  ...
  return (
    <div className="pt-4 sm:pt-8">
      <div className="flex items-center justify-between gap-3">
        ...
        <ImuSessionImport userId={userId} bikes={(bikes ?? []).map(({ id, name }) => ({ id, name }))} />
      </div>

      <div className="mt-6 space-y-4 pb-10">
        {(sessions ?? []).length === 0 && (
          <p className="rounded-xl border border-dashed border-border px-5 py-8 text-center text-sm text-muted-foreground">
            Ainda não há sessões. Importe um ficheiro JSON do sensor para começar.
          </p>
        )}
        ...
        {(sessions ?? []).map((session) => {
          ...
          return (
            ...
            >
              ...
              <p className="mt-1 font-display text-xl leading-tight font-bold">{session.name}</p>
              <div className="mt-2 flex items-end justify-between gap-3 text-sm text-muted-foreground">
                <p>
                  {formatDate(session.created_at)} · {Math.round(session.sample_rate_hz)} Hz ·{" "}
                  <span className="tabular-nums">{session.sample_count.toLocaleString("pt-PT")}</span> amostras
                </p>
                <p className="shrink-0 tabular-nums">{formatSessionTime(session.duration_ms)}</p>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">
                {session.max_g != null && (
                  <>
                    G máx <span className="font-medium text-foreground tabular-nums">{session.max_g.toFixed(2)}</span> ·{" "}
                  </>
                )}
                <span className="tabular-nums">{session.curve_count}</span> curvas ·{" "}
                <span className="tabular-nums">{session.jump_count}</span> saltos ·{" "}
                <span className="tabular-nums">{session.impact_count}</span> impactos
              </p>
