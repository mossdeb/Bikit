...
2. Miss → IA pesquisa a documentação oficial, **mas só para as categorias onde os fabricantes publicam planos de manutenção** (`AI_MAINTENANCE_SEARCH_CATEGORIES`, 2026-08-09): suspensão, amortecedor, espigão, quadro e elétrico. **Desde 2026-08-10 os desconhecidos vão numa única chamada em lote** (máx. 6 por lote, `searchMaintenanceBatchWithAI`): uma bicicleta nova custa 2 chamadas no total — bicicleta + lote — com o lookup no meio a esvaziar o lote à medida que o catálogo enche. Medido: Hightower nova em 29 s / 2 chamadas / 54k tokens, contra ~99 s / 4 chamadas / 165k da véspera. O ledger continua com uma linha por componente (tokens do lote divididos igualmente); o `source_url` do lote é a primeira citação da resposta, mais grosseiro que na era das chamadas individuais. Medido no primeiro teste real: pneus, selins, cockpits e travões devolveram sempre "sem documentação" e custavam o mesmo — mais de metade do gasto comprava respostas vazias; o filtro corta o pior caso de ~0,73 $ para ~0,35 $ por bicicleta nova. A app **guarda o perfil completo** (`km` / `hours` / `months`), e um `not_found` guarda um **perfil vazio** (cache negativo) para nunca se repetir a pesquisa.
